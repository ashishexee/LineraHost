...
export default function ApiTester() {
    ...
    const { performMutation } = useLinera();
    ...
    const handleSend = async (bypassPayment = false, overrideToken?: string) => {
        ...
        if (url.startsWith('linera://')) {
            try {
                setLoading(true);
                // Execute actual mutation via Linera Context
                // Expect body to be the GraphQL mutation string
                const result = await performMutation(body || url.replace('linera://', ''));

                setResponse({
                    status: 200,
                    statusText: "OK",
                    data: result,
                    headers: { 'source': 'linera-wasm' },
                    time: performance.now() - performance.now() // rough estimate
                });
            } catch (e: any) {
                ...
                setResponse({
                    status: 500,
                    statusText: "Error",
                    data: e.message || "Mutation Failed",
                    headers: {},
                    time: 0
                });
            } finally {
    ...
    const handlePayment = async () => {
        ...
        try {
            // Linera Payment Logic Stub
            // In the new architecture, payments are handled by the Linera microchain operations.
            // If the user needs to specificy an amount, it would be part of the 'operation' arguments.

            // For now, we simulate a successful payment or prompt the user using the wallet.
            if (typeof window.linera !== 'undefined') {
                // Example: window.linera.request({ method: 'linera_transfer', params: [...] })
                toast.info("Linera payment flow would trigger here. (Simulating success)");
            } else {
                toast.success("Payment Logic bypassed for Linera Testnet Demo");
            }
            ...
            <div className="bg-[#1A1B23] border border-white/10 rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-3 bg-black/20 p-2.5 rounded-lg border border-white/5 group hover:border-white/10 transition-colors">
                        ...
                        <span className="flex-1 text-xs text-gray-300 font-mono truncate" title="https://linera-host-gateway.vercel.app/run/0xda684a91a506f3e303834c97784dee2b31bbc6bc/peerhostfree/hello">
                            .../peerhostfree/hello
                        </span>
                        <button
                            onClick={() => copyToClipboard('https://linera-host-gateway.vercel.app/run/0xda684a91a506f3e303834c97784dee2b31bbc6bc/peerhostfree/hello', 'free')}
                    ...
                    <div className="flex items-center gap-3 bg-black/20 p-2.5 rounded-lg border border-white/5 group hover:border-white/10 transition-colors">
                        ...
                        <span className="flex-1 text-xs text-gray-300 font-mono truncate" title="https://linera-host-gateway.vercel.app/run/0xda684a91a506f3e303834c97784dee2b31bbc6bc/peerhostefjb/hello">
                            .../peerhostefjb/hello
                        </span>
                        <button
                            onClick={() => copyToClipboard('https://linera-host-gateway.vercel.app/run/0xda684a91a506f3e303834c97784dee2b31bbc6bc/peerhostefjb/hello', 'paid')}
                            className="p-1.5 text-gray-500 hover:text-white hover:bg-white/10 rounded-md transition-all"
                            title="Copy & Use"
                        >
            ...
            <div className="bg-[#1A1B23] border border-white/10 rounded-xl p-6 space-y-6">
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <input
                        ...
                        placeholder="https://linera-host-gateway.vercel.app/..."
